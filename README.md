...
A focused LibreChat-style MVP for Smartyfy Chat: paste or configure a model API key, fetch available models, and chat through the OpenAI-compatible API.

## What is included

- Live `/v1/models` discovery with searchable model metadata.
...
- Supported request controls: `temperature`, `top_p`, `max_tokens`, `seed`, and `stop`.
...
The app allows these OpenAI-compatible provider endpoints:

- `https://crof.ai/v1`
- `https://crof.ai/v2`

The provider docs describe OpenAI-compatible chat completions, streaming, reasoning deltas, tools, vision content, `/v1/models`, and the supported parameters used here. The model picker is not hardcoded; it calls the selected endpoint's `/models` route after a key is configured and refreshes the live list periodically.
