...
| `server/websearch/index.js` | `WebSearchOrchestrator` — provider chain (SearXNG → Jina → Brave) with per-provider circuit breaker, two-tier LRU+Supabase cache via `SearchCache`, and `readUrl` for direct page reads through Jina's `r.jina.ai`. | `./brave.js`, `./cache.js`, `./jina.js`, `./searxng.js` |
...
| Path | Owns |
|---|---|
| `worker/worker.py` | The Python worker entry point. `Processor` polls `klui_claim_document_job`, downloads from R2, dispatches `extract_job` / `create_job` / `edit_job` / `export_job`. The `Supabase`/`R2`/`JinaEmbeddings` helper classes live inside this file. |
...
| `messages` | `id`, `user_id`, `conversation_id`, `role` (`system`/`user`/`assistant`/`tool`), `content jsonb` (can be string or parts array), `model`, `reasoning`, `tool_calls jsonb`, `finish_reason`, `error`, `metadata jsonb`, `created_at`. | `metadata` carries the council/websearch/documents/research context per message. |
...
| `document_files` | `id`, `attachment_id` (unique), `user_id`, `conversation_id`, `message_id`, `kind` (`pdf`/`docx`/`xlsx`/`pptx`/`csv`/`tsv`), `source` (`upload`/`generated`/`edited`/`exported`), `parent_document_id` (self-FK for versions), `version_no`, `source_etag`, `processing_status` (`pending`/`processing`/`ready`/`failed`), counts (`page_count`/`word_count`/`sheet_count`/`used_cell_count`), `extraction_key`, `preview_key`, `metadata`, `error`. | Cascades with `attachments` and `conversations`. |
...
| `document_jobs` | `id`, `user_id`, `document_file_id`, `conversation_id`, `message_id`, `job_type`, `status` (`queued`/`running`/`succeeded`/`failed`/`expired`), `priority`, `attempt_count`, `worker_id`, `lease_until`, `input jsonb`, `output jsonb`, `error jsonb`, timestamps. | Queued with `document_jobs_claim_idx` and `document_jobs_lease_idx`. |
