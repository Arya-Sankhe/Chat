...
} from "../server/saas/writingStyles.js";
...
test("each writing style adds a distinct server-side skill instruction", () => {
  const styles = ["learning", "concise", "explanatory", "formal"];
...
test("response length adjustments are allowlisted and carry the previous answer", () => {
  assert.equal(normalizeResponseAdjustment("LONGER"), "longer");
  assert.equal(normalizeResponseAdjustment("verbose"), "");
  const prompt = withResponseAdjustmentSystemPrompt("Base prompt", "shorter", "Original answer");
  assert.match(prompt, /substantially shorter/);
