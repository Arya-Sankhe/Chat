-- Let the last request that starts with a positive balance finish normally.
-- Its full reservation blocks concurrent work, and settlement still refuses
-- any provider cost above the reservation ceiling.
...
begin
  ...
  -- A positive balance authorizes one final request at the normal ceiling.
  -- Adding its reservation immediately closes the gate to concurrent calls.
  if v_limit <= 0 or v_week.api_credit_used + v_week.api_credit_reserved >= v_limit then
    return jsonb_build_object(
