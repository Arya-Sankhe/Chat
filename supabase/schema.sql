...
create table if not exists public.usage_api_weekly (
  ...
  api_credit_reserved numeric(18,8) not null default 0,
  updated_at timestamptz not null default now(),
...
create table if not exists public.usage_api_events (
  ...
  reserved_credits numeric(18,8) not null default 0,
...
-- either the global 'funded_inference_disabled' key or the per-user
-- 'funded_inference_disabled:<user_id>' key is set.
...
-- Full-text chat search (see migration 20260816234517_search_messages_fts).
-- Extractor + GIN expression index + search RPC; no stored tsvector column.
