export function renderMindMap(title, content, escapeHtml) {
  ...
  for (const line of String(content || "").split("\n")) {
    ...
    const node = { label: (heading?.[2] || bullet[2]).trim().replace(/^#{1,6}\s+/, ""), children: [] };
  ...
  return `<div class="dojo-mindmap"><div class="dojo-map-root">${escapeHtml(title)}</div><div class="dojo-map-scroll"><div class="dojo-map-canvas">${render(root.children)}</div></div></div>`;
}
