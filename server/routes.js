...
async function handleCreateResearch(req, res, config) {
...
async function handleResearchStatus(req, res, config, runId) {
...
async function handleCancelResearch(req, res, config, runId) {
...
async function handleResearchReport(req, res, config, runId) {
  if (req.method !== "GET") throw new HttpError(405, "Method not allowed.");
  const context = await requireChatContext(req, config);
  const run = await context.db.getResearchRun(context.user.id, runId, { signal: req.signal });
  if (!run) throw new HttpError(404, "Research run not found.");
